...
%% (6) retrain least squares algorithm with new feature vector

%Initialized
M_values = [20, 50, 1000, 5000, 10000];
RandFeatures = length(M_values);

%Comparison values 
ER  = zeros(1,RandFeatures);
FPR = zeros(1,RandFeatures);
FNR = zeros(1,RandFeatures);

%Compute new feature vector for each M
for i = 1:RandFeatures
    M = M_values(i);
    
    %R = randi(h*w, M, 1); %create random matrix containing {-1,1}
    R = sign(randn(M, h*w));

    %Compute new features
    %A_tr_random = Xflat_tr(R, :).'; %Ntr x M
    %A_te_random = Xflat_te(R, :).'; %Ntr x M
    z_train = max(0, Xflat_tr.' * R.');   % (Ntr x M)
    z_test  = max(0, Xflat_te.' * R.');   % (Nte x M)

    %(7) Retrain least squares 
    %new_thetas = A_tr_random \ y_tr;
    theta_rand = z_train \ y_tr;

    %Test new features
    %scores_random = A_te_random * new_thetas;
    %y_predict = sign(scores_random);
    y_predict = sign(z_test * theta_rand);
    y_predict(y_predict == 0) = -1; %ties

    %Compute metrics for new thetas
    Comparisons = label_comparisons(y_predict, y_te);
    ER(i)  = Comparisons.ER;
    FPR(i) = Comparisons.FPR;
    FNR(i) = Comparisons.FNR;
end

%Summarized and plotted results
Results = table(M_values(:), ER(:), FNR(:), FPR(:), 'VariableNames', ...
    {'M', 'Error Rate', 'False Negative Rate', 'False Positive Rate'});

disp('----Random Feature Metrics----');
disp(Results);

figure;
plot(M_values, ER, '-o', 'LineWidth', 1.5); hold on;
plot(M_values, FNR, '-s', 'LineWidth', 1.5);
plot(M_values, FPR, '-d', 'LineWidth', 1.5);
hold off;

xlabel('Number of Random Features (M)');
ylabel('Rate');
title('Error, False Negative, and False Positive Rates vs. M');
legend({'Error Rate', 'False Negative Rate', 'False Positive Rate'}, 'Location', 'best');
grid on;


%Each column of R corresponds to one feature
theta_img_rand = R.' * theta_rand;  % (400 x 1)
theta_img_rand = reshape(theta_img_rand, h, w);

figure;
imagesc(theta_img_rand); axis image off; colorbar;
title(sprintf('\\theta (random features, M=%d) mapped to pixel locations', M));


%% HELPER Function %%

%Compute error, false negative, and false positive rate
function Comparisons = label_comparisons(y_hat, y_true)
    err_rate = mean(y_hat ~= y_true);

    neg_idx = (y_true == -1);
    pos_idx = (y_true == 1);
    FPR = mean(y_hat(neg_idx) == 1);
    FNR = mean(y_hat(pos_idx) == -1);

    Comparisons.ER = err_rate;
    Comparisons.FPR = FPR;
    Comparisons.FNR = FNR;
end