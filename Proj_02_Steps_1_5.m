...
for i = 1:RandFeatures
    ...
    R = sign(rand(M, M0)); %create random matrix containing {-1,1}
    
    %Compute new features
    z_train = compute_random_features(A_tr, R);
    z_test = compute_random_features(A_te, R);

    %(7) Retrain least squares 
    new_thetas = Least_squares_training(z_train, y_tr); 

    %Test new features
    y_predict = sign(z_test * new_thetas);
...
function Comparisons = label_comparisons(predicted_label, true_label)
    ...
    %Overall error rate
    Comparisons.ER = mean(y_hat ~= y_te);
    ...
    %Avoid NaN
    if any(neg_idx)
        Comparisons.FPR = mean(y_hat(neg_idx) == 1);
    else
        Comparisons.FPR = NaN;
    end

    if any(pos_idx)
        Comparisons.FNR = mean(y_hat(pos_idx) == -1);
    else
        Comparisons.FNR = NaN;
    end
end
...
function Z = compute_random_features(X, R)
...
%Training function 
function theta = Least_squares_training(Z, Y)
    theta = (Z' * Z) \ (Z' * Y);
end